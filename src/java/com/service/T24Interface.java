 ...
 * and open the templ9ate in the editor.
 */
package com.service;
    ...
    public T24Interface()
{
    ...
    //    ServletContext se = getServletContext();
        ...
        propertiesfile = classLoader.getResourceAsStream("com/service/interfacelogger.properties");
//watcherEJB ejbRef = (watcherEJB) new InitialContext().lookup("java:comp/env/myEJB");
   // ejbRef.ejbMethod();
String a ="b";
...
private WebServiceLogger getServiceLogger(String filename){
    ...
    public ISOResponse ProcessISO(@WebParam(name = "ISOMessage") String ISOMessage) {
        ...
        //Mnemonic Product Account Id CLASS-POSNEG Ccy Account Officer
         if(watcherthread.getState() == State.NEW){
          ...
          public void run() {
              ...
              try{
                       watchDirectoryPath(new File(listeningDir).toPath(),new FileLoggerHandler("atm_monitor",LogDir,propertiesfile));
              } catch(Exception v)
              {
       ...
       String lengthString =  ISOMessage.substring(0, 4);
       ISOMessage = ISOMessage.substring(4);
       int msglength = ISOMessage.length();
       int speclenght = Integer.parseInt(lengthString);
        
       if(msglength!=speclenght){
             
         throw new  Exception("Unsupported Message Format. Expected  message lenght is ("+ speclenght+") found ("+msglength+")");
        ...
        message.setIsSuccessful(true);
          ...
          message.setErrorMessgae("Failed to process ISO Message because: "+d.getMessage());
    ...
    public TransactionResponse DirectDebit(@WebParam(name = "debit") TransactionRequest debit) {
